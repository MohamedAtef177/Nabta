...
types = ["Grape___Black_rot", "Grape___Esca_(Black_Measles)", "Grape___healthy",
...
le = len(training_data[2])
for i in range(0, le):
    for j in range(1):
        w = aug(training_data[2][i][0].reshape(-1, 200, 200, 3))
        training_data[2].append([w[0], 2])
...
pickle_out = open("X_train_grape.pickle", "wb")
pickle.dump(X_train, pickle_out)
pickle_out.close()

pickle_out = open("Y_train_grape.pickle", "wb")
pickle.dump(Y_train, pickle_out)
pickle_out.close()

pickle_out = open("X_test_grape.pickle", "wb")
pickle.dump(X_test, pickle_out)
pickle_out.close()

pickle_out = open("Y_test_grape.pickle", "wb")
