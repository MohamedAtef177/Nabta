...
types = ["Potato___Early_blight", "Potato___healthy", "Potato___Late_blight"]
...
le = len(training_data[1])
for i in range(0, le):
    for j in range(5):
        w = aug(training_data[1][i][0].reshape(-1, 200, 200, 3))
        training_data[1].append([w[0], 1])
...
pickle_out = open("X_train_Potato.pickle", "wb")
pickle.dump(X_train, pickle_out)
pickle_out.close()

pickle_out = open("Y_train_Potato.pickle", "wb")
pickle.dump(Y_train, pickle_out)
pickle_out.close()

pickle_out = open("X_test_Potato.pickle", "wb")
pickle.dump(X_test, pickle_out)
pickle_out.close()

pickle_out = open("Y_test_Potato.pickle", "wb")
