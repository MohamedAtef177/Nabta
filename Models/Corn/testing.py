...
X_test = pickle.load(open("X_test_corn.pickle", "rb"))
Y_test = pickle.load(open("Y_test_corn.pickle", "rb"))


model = models.load_model("corn_diseases_inception_V3_1")
...
pickle_out = open("test_result_corn_diseases.pickle", "wb")
pickle.dump(yhat, pickle_out)
pickle_out.close()
''' 
yhat = pickle.load(open("test_result_corn_diseases.pickle", "rb"))
...
precision = np.sum(true_pos / np.sum(cm1, axis=0))
recall = np.sum(true_pos / np.sum(cm1, axis=1))
print("recall 1 :", recall)
print("precision 1 :", precision)

plot_confusion_matrix(cm1,
                      ["Gray_leaf_spot", "Common_rust_", "healthy", "Northern_Leaf_Blight"],
                      title='Corn Confusion matrix',
                      cmap=None,
                      normalize=False)
